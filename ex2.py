...
def merge(first_dict, second_dict, new_dict):
    ...
    a=loop(first_dict,second_dict,new_dict)
    for key in a:
...
def conc(list_a, list_b):
    ...
    for element_i in [list_a, list_b]:
        for element_in_i in element_i:
            if element_in_i not in flattened:
                flattened.append(element_in_i)#y
    return flattened

def loop(first_d,second_d,ndict):
    for key in [first_d,second_d]:
        for ky in key:
            ndict={x:True for x in ky}
            ndict.append(ky)
    return ndict
